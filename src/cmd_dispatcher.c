...
CmdDispatchResult_t cmd_dispatch_string(const char *str, CmdSource_t src, int32_t cid) {
    ...
    if (strncmp(str, "stop_imm", 8) == 0 || strncmp(str, "STOP_IMM", 8) == 0 ||
        ...
        bool ok = pump_hmi_execute(HMI_ACTION_STOP, 0.0f, 0.0f);
        return dispatch_result(ok, ok ? "ok" : "queue_full", src, (uint8_t)HMI_ACTION_STOP);
    }

    // ------------------------------------------------------------------
    // FSM commands — validated against current Core 1 state
    // ------------------------------------------------------------------
    if (strncmp(str, "fsm_home,", 9) == 0) {
        bool ok = pump_hmi_execute(HMI_ACTION_HOME, (float)atof(str + 9), 0.0f);
        if (!ok) printf("[CMD]: Rechazado en estado: %s\n", get_state_name(pump_hmi_get_fsm_state()));
        return dispatch_result(ok, ok ? "ok" : "invalid_state", src, (uint8_t)HMI_ACTION_HOME);
    }
    if (strncmp(str, "fsm_search,", 11) == 0) {
        bool ok = pump_hmi_execute(HMI_ACTION_SEARCH_SYRINGE, (float)atof(str + 11), 0.0f);
        if (!ok) printf("[CMD]: Rechazado en estado: %s\n", get_state_name(pump_hmi_get_fsm_state()));
        return dispatch_result(ok, ok ? "ok" : "invalid_state", src, (uint8_t)HMI_ACTION_SEARCH_SYRINGE);
    }
    if (strncmp(str, "fsm_dispense,", 13) == 0) {
        ...
        if (sscanf(str + 13, "%f,%f", &target, &vel) != 2) {
            ...
            return dispatch_result(false, "bad_format", src, (uint8_t)HMI_ACTION_START_DISPENSE);
        }
        bool ok = pump_hmi_execute(HMI_ACTION_START_DISPENSE, target, vel);
        if (!ok) printf("[CMD]: Rechazado en estado: %s\n", get_state_name(pump_hmi_get_fsm_state()));
        return dispatch_result(ok, ok ? "ok" : "invalid_state", src, (uint8_t)HMI_ACTION_START_DISPENSE);
    }
    if (strncmp(str, "fsm_search_eot", 14) == 0) {
        bool ok = pump_hmi_execute(HMI_ACTION_SEARCH_EOT, 0.0f, 0.0f);
        if (!ok) printf("[CMD]: Rechazado en estado: %s\n", get_state_name(pump_hmi_get_fsm_state()));
        return dispatch_result(ok, ok ? "ok" : "invalid_state", src, (uint8_t)HMI_ACTION_SEARCH_EOT);
    }
    if (strncmp(str, "fsm_reset", 9) == 0) {
        bool ok = pump_hmi_execute(HMI_ACTION_RESET, 0.0f, 0.0f);
        return dispatch_result(ok, ok ? "ok" : "queue_full", src, (uint8_t)HMI_ACTION_RESET);
    }
    if (strncmp(str, "fsm_cont", 8) == 0) {
        bool ok = pump_hmi_execute(HMI_ACTION_CONTINUE_DISPENSE, 0.0f, 0.0f);
        if (!ok) printf("[CMD]: Rechazado en estado: %s\n", get_state_name(pump_hmi_get_fsm_state()));
        return dispatch_result(ok, ok ? "ok" : "invalid_state", src, (uint8_t)HMI_ACTION_CONTINUE_DISPENSE);
    }
    if (strncmp(str, "fsm_occ_rel", 11) == 0) {
        bool ok = pump_hmi_execute(HMI_ACTION_OCC_RELEASE, 0.0f, 0.0f);
        if (!ok) printf("[CMD]: Rechazado en estado: %s\n", get_state_name(pump_hmi_get_fsm_state()));
        return dispatch_result(ok, ok ? "ok" : "invalid_state", src, (uint8_t)HMI_ACTION_OCC_RELEASE);
    }
    if (strncmp(str, "fsm_resume", 10) == 0) {
        bool ok = pump_hmi_execute(HMI_ACTION_RESUME, 0.0f, 0.0f);
        if (!ok) printf("[CMD]: Rechazado en estado: %s\n", get_state_name(pump_hmi_get_fsm_state()));
        return dispatch_result(ok, ok ? "ok" : "invalid_state", src, (uint8_t)HMI_ACTION_RESUME);
    }
    if (strncmp(str, "fsm_calibrate", 13) == 0) {
        bool ok = pump_hmi_execute(HMI_ACTION_CALIBRATE, 0.0f, 0.0f);
        if (!ok) printf("[CMD]: Rechazado en estado: %s\n", get_state_name(pump_hmi_get_fsm_state()));
        return dispatch_result(ok, ok ? "ok" : "invalid_state", src, (uint8_t)HMI_ACTION_CALIBRATE);
    }
